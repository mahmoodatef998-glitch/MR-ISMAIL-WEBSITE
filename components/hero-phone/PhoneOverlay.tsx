...
export function PhoneOverlay({ progress }: Props) {
  ...
  // Scene 1 (hand, center p≈0.12): label at bottom
  const s1Op = useTransform(progress, [0.04, 0.10, 0.14, 0.18], [0, 1, 1, 0])
  const s1Y  = useTransform(progress, [0.04, 0.10], [14, 0])

  // Scene 2 (macro, center p≈0.30): camera specs at bottom
  const s2Op = useTransform(progress, [0.27, 0.33, 0.38, 0.43], [0, 1, 1, 0])

  // Scene 3 (orange back, center p≈0.50): Arabic headline bottom
  const s3Op    = useTransform(progress, [0.47, 0.52, 0.58, 0.62], [0, 1, 1, 0])
  const s3Y     = useTransform(progress, [0.47, 0.52], [28, 0])
  const s3SubOp = useTransform(progress, [0.49, 0.54, 0.58, 0.62], [0, 1, 1, 0])

  // Scene 4 (full reveal, center p≈0.70): color label top
  const s4Op = useTransform(progress, [0.67, 0.72, 0.76, 0.80], [0, 1, 1, 0])

  // Scene 5 (architecture, p≈0.88-1.0): CTA bottom
  ...
  return (
    <div className="absolute inset-0 z-10 pointer-events-none select-none">
      ...
      >
        ...
        <p className="text-white/52 text-xs tracking-[0.22em] uppercase"
      ...
      <motion.div
        ...
        style={{ opacity: s2Op }}
      >
        <p className="text-white/75 text-2xl md:text-3xl"
      ...
      >
        <h1 className="font-display text-6xl md:text-8xl gold-text text-center"
      ...
      <motion.div
        ...
        style={{ opacity: s4Op }}
      >
        <p className="text-[9px] uppercase tracking-[0.52em]"
          style={{ fontFamily: 'var(--font-sans)', color: 'rgba(196,146,42,0.55)' }}>
          New
        </p>
        <p className="text-white/62 text-base md:text-lg tracking-[0.14em] uppercase mt-1.5"
          style={{ fontFamily: 'var(--font-sans)', fontWeight: 300 }}>
          Desert Titanium
        </p>
      </motion.div>
      ...
      >
        <h2 className="font-display text-3xl md:text-4xl text-white/88 text-center"
          style={{ letterSpacing: '-0.015em' }}>
          اكتشف المجموعة
        </h2>
        <p className="text-white/32 text-sm text-center"
          style={{ fontFamily: 'var(--font-sans)', fontWeight: 300 }}>
