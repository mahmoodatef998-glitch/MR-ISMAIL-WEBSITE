      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-24">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white leading-tight mb-4">
            {lang === 'en' ? (
              <>Built for Businesses<br /><span className="gold-text">That Demand More</span></>
            ) : (
              <>مبني للشركات التي<br /><span className="gold-text">تطلب الأفضل</span></>
            )}
          </h2>
          <p className="text-gray-500 text-sm leading-relaxed">
            {lang === 'en'
              ? 'Fifteen years of wholesale experience distilled into a supply chain that just works.'
              : 'خمسة عشر عامًا من الخبرة في الجملة تتجلى في سلسلة توريد تعمل بكفاءة.'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {FEATURES.map(({ icon: Icon, en, ar }, i) => {
            ...
            return (
              <div
                key={i}
                className="group relative bg-[#0a1628] border border-white/[0.05] rounded-2xl p-7 hover:border-[#c8a96e]/25 hover:bg-[#0d1e36] transition-all duration-300 overflow-hidden"
              >
                ...
                <div className="relative">
                  <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-[#c8a96e]/10 border border-[#c8a96e]/15 mb-5 group-hover:bg-[#c8a96e]/15 group-hover:border-[#c8a96e]/30 transition-all duration-300">
                    <Icon className="w-5 h-5 text-[#c8a96e]" />
                  </div>
                  <h3 className="text-white font-bold text-base mb-2.5 group-hover:text-[#c8a96e] transition-colors duration-200">
                    {content.title}
                  </h3>
                  <p className="text-gray-500 text-sm leading-relaxed">
                    {content.desc}
